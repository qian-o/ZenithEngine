...
struct VSInput
{
    ...
    [[vk::location(5)]] int NodeIndex : TEXCOORD2;
    [[vk::location(6)]] int ColorMapIndex : TEXCOORD3;
    [[vk::location(7)]] int NormalMapIndex : TEXCOORD4;
};
...
{
    float4 position = mul(nodeTransform[input.NodeIndex], float4(input.Position, 1.0));
    
    VSOutput output;
    
    output.Position = mul(frame.Projection, mul(frame.View, position));
    output.Normal = normalize(mul(nodeTransform[input.NodeIndex], float4(input.Normal, 0.0)).xyz);
